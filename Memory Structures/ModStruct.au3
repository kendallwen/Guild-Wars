...
Func LogItems()
   ...
   For $i = 1 To 4
	  For $j = 1 To $BAG_SLOT[$i-1]
		 ...
		 While(StringLen($modStruct)>0)
			Local $temp = StringLeft($modStruct, 8)
			FileWriteLine("Log.txt", $temp & @CRLF)
			$modStruct = StringTrimLeft($modStruct, 8)
		 WEnd
		 Out1($j)
		 Sleep(1000)
